...
def lambda_handler(event, context):
  ...
  curl_cmd = f"""curl -XPOST {cf_obj["url"]} -H'Content-Type: application/json' --data '{cf_obj["data"]}'"""
  logger.debug(f"If failed, manually send: {curl_cmd}")
  userid = None
  ...
  try:
    ...
    if event['RequestType'] != 'Delete':
      ...
      try:
        ...
        senders.append(
            f"""dvsa.{os.environ["ACCOUNT_ID"]}.{''.join(userid.split('-'))}@1secmail.com"""
        ) if userid is not None else None
      ...
      for b in buckets:
        logger.debug(f"Empyting bucket: {b}")
        try:
          bucket = s3.Bucket(b)
      ...
      for b in buckets:
        logger.debug(f"Deleting bucket: {b}")
        try:
          bucket = s3.Bucket(b)
          bucket.delete()
        except:
  ...
  for root, dirs, files in os.walk('dist_s3/images'):
    for file in files:
      s3.meta.client.upload_file(
          f'dist_s3/images/{file}',
          os.environ["CLIENT_BUCKET"],
          f'images/{file}',
      )
...
def deleteLogGroup():
  ...
  for lg in lgs:
    ...
    if lg_name.startswith("/aws/lambda/DVSA-"):
      ...
      logger.debug(f"Deleted LogGroup: {lg_name}")
  return
...
def verifySESIdentities(emails):
  ...
  logger.debug(f"verifying: {','.join(emails)}")
  for email in emails:
    try:
      response = ses.verify_email_identity(
        EmailAddress=email
      )
      logger.debug(f"sent verify-email to: {email}")
      if email.endswith("1secmail.com"):
        logger.debug(f"trying to auto-verify: {email}")
        sleep(6)
          ...
          if aws_msg_list := [
            ...
            if req.status < 300:
              ...
              if (startpoint != -1 and endpoint != -1):
                ...
                logger.debug(f"{email}: Auto-verified!")
...
def createAdminUser():
  ...
  logger.debug(f"creating admin user for: {admin_email}")
  try:
      ...
      for user in users:
        for att in user.get("Attributes", []):
          if att["Name"] == "email" and att["Value"] == admin:
            ...
            logger.debug(f"found userid: {userid}")
            break
    else:
      logger.exception("[INIT-ERROR]")
      return
